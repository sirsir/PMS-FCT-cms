...
function RenderCssCal(bNewCal) {
	...
	if (Cal.Scroller=="DROPDOWN") {
		...
		for (i = (dtToday.getFullYear() - 10); i < (dtToday.getFullYear() + 5);i++)	{
			if (i==Cal.Year)
				SelectStr="Selected";
			else
				SelectStr="";	
...
function closewin(id) {
   ...
   var CalTextId = document.getElementById(id + "_text");

   if (CalTextId != null) CalTextId.focus();
   if (CalTextId == null && CalId != null) CalId.focus();
...
function callback(id, datum) {
    var CalId = document.getElementById(id);CalId.value=datum;
    var CalTextId = document.getElementById(id + "_text");
    ...
    if (CalId.onchange != null) CalId.onchange();
    winCal.style.visibility='hidden';
}

function setDateTimeRange(element_id){
    if (element_id.search(/from/i) == -1){
       second_element_id = element_id.replace("to", "from");
    }
    else{
      second_element_id = element_id.replace("from", "to");
    }
    first_element = document.getElementById(element_id);
    first_element_text = document.getElementById(element_id + "_text");
    second_element = document.getElementById(second_element_id);
    second_element_text = document.getElementById(second_element_id + "_text");

    if (second_element.value == "") {
       second_element.value = first_element.value
       second_element_text.value = first_element_text.value
    }
    else{
        if ((element_id.search(/from/i) != -1) && (first_element.value > second_element.value)){
            second_element.value = first_element.value
            second_element_text.value = first_element_text.value
        }
        else if ((element_id.search(/to/i) != -1) && (first_element.value < second_element.value)){
            second_element.value = first_element.value
            second_element_text.value = first_element_text.value 
        }
    }
