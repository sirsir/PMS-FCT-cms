class CustomFields::TextArea < CustomField

  DEFAULT_MAX_LENGTH = 200

  class << self

    #   CustomFields::TextArea.name_prefix -> string
    #
    # Get the default name prefix
    #   CustomFields::TextArea.name_prefix #=> txa
    def name_prefix
      'txa'
    end
  end
  ...
  # Get option
  #   custom_field.option #=> {:numeric => "true", :symbols => [#CustomFields1, #CustomFields2, ...]}
  def option
    @option ||= {
    ...
    when String
      ...
      if options[:throw_exception]
        ...
        if self.option[:symbols].to_s.empty?
          ...
          #CHECK if it cantain other char
          str_stranger = cell_value
          ...
          unless str_stranger.empty?
            ...
            raise_message << ", Thai, or Japanese" if self.option[:non_english].to_s == 'true'
