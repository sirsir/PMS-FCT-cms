...
ENV['name_cf_id'] = "186"
class AddProductSeriesAcc < ActiveRecord::Migration
  def self.up
    ...
    cell = Cell.new(:field_id => name_cf.id, :value => 'ACC')
    row.cells << cell
  ...
  def self.down
     ...
     product_type_row = product_series.rows.collect{|r| r if r.cell(name_cf.id).value == "ACC"}.compact[0]
